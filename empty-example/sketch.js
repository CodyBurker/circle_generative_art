...
function drawEdgeCircle(mainCircleRadius){
  ...
  drawDots(circvec, circradius, mainCircleRadius);


}

// Draw 3 dots at the edge of the circle
function drawDots(circleCenter, circleRadius,mainCircleRadius){
  vec = createVector(circleCenter.x - width/2, circleCenter.y - height/2);
  factor = 0.5;
  ...
  vec2 = createVector(vec.x - width/2, vec.y - height/2);
  vec3 = vec2.copy();
